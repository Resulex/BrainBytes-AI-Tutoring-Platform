...
function MessageBubble({ message, onVisible, onSuggestionClick }) {
  ...
  const renderText = (text) => {
    ...
    html = html.replace(/^\s*[-*]\s(.+)$/gm, '<li style="margin:3px 0">$1</li>');
    html = html.replace(
      /(<li[\s\S]*?<\/li>)/g,
  ...
  return (
    ...
    >
      ...
      <div style={styles.messageMeta}>
        <span>
          {message.isUser ? 'You' : 'AI Tutor'} • {time}
        </span>
        {!message.isUser && (
  ...
  const handleSubmit = async (e) => {
    ...
    if (isConnected && socket) {
      ...
      try {
        ...
        setMessages((prev) =>
          prev.map((msg) => (msg._id === tempId ? { ...msg, _failed: true } : msg)),
        );
      } finally {
        setIsAiTyping(false);
      }
        ...
        <div style={styles.inputRow}>
          ...
          <div style={{ flex: 1, display: 'flex' }}>
            ...
            <button
              ...
              style={{
                ...styles.sendButton,
                opacity: isAiTyping || !newMessage.trim() ? 0.6 : 1,

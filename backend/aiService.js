...
async function generateResponse(question, preferredSubject = null, context = null) {
  ...
  const frustrationKeywords = ['not working', 'wrong', 'bad', 'stupid', 'help', 'confused', 'don\'t understand'];
  ...
  if (preferredSubject && ['math', 'science', 'history', 'general'].includes(preferredSubject)) {
    ...
    if (lowerQuestion.includes('math') || lowerQuestion.includes('algebra') || lowerQuestion.includes('calculus')) {
      ...
      category = 'science';
    } else if (lowerQuestion.includes('history') || lowerQuestion.includes('war') || lowerQuestion.includes('century')) {
      category = 'history';
    } else {
      category = 'general';
    }
    ...
    for (const model of MODELS) {
      try {
        ...
        if (response.status === 401 || response.status === 403) {
          ...
          throw new Error(`Hugging Face API authentication failed (${response.status}). Check your HUGGINGFACE_TOKEN.`);
        }

        if (!response.ok) {
          const errorBody = await response.text();
          console.log(`Model [${model}] failed: ${response.status} - ${errorBody.substring(0, 200)}`);
        ...
        if (result.error) {
          console.log(`Model [${model}] returned error: ${JSON.stringify(result.error).substring(0, 200)}`);
          continue;
        }
        ...
        if (modelError.name === 'AbortError') {
          ...
          console.log(`Model [${model}] failed: ${modelError.message.split('\n')[0]}, trying next...`);
        }
...
function getDetailedResponse(category, question, questionType) {
  ...
  if (category === 'science') {
    ...
    if (lowerQuestion.includes('science') || lowerQuestion.includes('scientific method')) {
      if (lowerQuestion.includes('branch') || lowerQuestion.includes('field') || lowerQuestion.includes('type')) {
    ...
    if (lowerQuestion.includes('gravity') || lowerQuestion.includes('fall') || lowerQuestion.includes('weight')) {
    ...
    if (lowerQuestion.includes('photosynthesis') || lowerQuestion.includes('plant') && lowerQuestion.includes('food')) {
    ...
    if (lowerQuestion.includes('force') || lowerQuestion.includes('motion') || lowerQuestion.includes('newton')) {
    ...
    if (lowerQuestion.includes('biology') || lowerQuestion.includes('what is biology') || lowerQuestion.includes('living')) {
    ...
    if (lowerQuestion.includes('atom') || lowerQuestion.includes('molecule') || lowerQuestion.includes('element')) {
    ...
    if (lowerQuestion.includes('weather') || lowerQuestion.includes('climate') || lowerQuestion.includes('rain')) {
    ...
    if (lowerQuestion.includes('body') || lowerQuestion.includes('human') || lowerQuestion.includes('heart') || lowerQuestion.includes('brain')) {
    ...
    if (lowerQuestion.includes('ecosystem') || lowerQuestion.includes('habitat') || lowerQuestion.includes('environment')) {
  ...
  if (category === 'math') {
    ...
    if (/^\d+\s*[+\-*\/]\s*\d+/.test(lowerQuestion) || /^what\s+(is|does)\s+\d+\s*[+\-*\/]\s*\d+/.test(lowerQuestion)) {
      try {
      ...
      if (lowerQuestion.includes('+') || lowerQuestion.includes('add') || lowerQuestion.includes('sum') || lowerQuestion.includes('plus')) {
      ...
      if (lowerQuestion.includes('*') || lowerQuestion.includes('×') || lowerQuestion.includes('multiply') || lowerQuestion.includes('times') || lowerQuestion.includes('product')) {
      ...
      if (lowerQuestion.includes('/') || lowerQuestion.includes('÷') || lowerQuestion.includes('divid') || lowerQuestion.includes('quotient')) {
    ...
    if (lowerQuestion.includes('add') || lowerQuestion.includes('sum') || lowerQuestion.includes('plus')) {
    ...
    if (lowerQuestion.includes('subtract') || lowerQuestion.includes('minus') || lowerQuestion.includes('difference') || lowerQuestion.includes('take away')) {
    ...
    if (lowerQuestion.includes('multiply') || lowerQuestion.includes('times') || lowerQuestion.includes('product')) {
    ...
    if (lowerQuestion.includes('divid') || lowerQuestion.includes('quotient') || lowerQuestion.includes('shared')) {
    ...
    if (lowerQuestion.includes('fraction') || lowerQuestion.includes('numerator') || lowerQuestion.includes('denominator')) {
    ...
    if (lowerQuestion.includes('algebra') || lowerQuestion.includes('equation') || lowerQuestion.includes('solve for') || lowerQuestion.includes('variable') || lowerQuestion.includes('x +') || lowerQuestion.includes('x =') || lowerQuestion.includes('find x')) {
    ...
    if (lowerQuestion.includes('geometry') || lowerQuestion.includes('area') || lowerQuestion.includes('perimeter') || lowerQuestion.includes('volume') || lowerQuestion.includes('shape')) {
    ...
    if (lowerQuestion.includes('pythagorean') || lowerQuestion.includes('right triangle') || lowerQuestion.includes('hypotenuse')) {
  ...
  if (category === 'history') {
    ...
    if (lowerQuestion.includes('president') || lowerQuestion.includes('presidente') || (lowerQuestion.includes('philippine') && lowerQuestion.includes('leader'))) {
    ...
    if (lowerQuestion.includes('world war') || lowerQuestion.includes('wwii') || lowerQuestion.includes('ww2') || lowerQuestion.includes('japan') && lowerQuestion.includes('philippine')) {
    ...
    if (lowerQuestion.includes('spanish') || lowerQuestion.includes('colon') || lowerQuestion.includes('galleon')) {
    ...
    if (lowerQuestion.includes('independence') || lowerQuestion.includes(' freedom') || lowerQuestion.includes('liberation')) {
    ...
    if (lowerQuestion.includes('hero') || lowerQuestion.includes('andres') || lowerQuestion.includes('bonifacio')) {
  ...
  if (questionType === 'definition') {
    const definitions = {
      ...
      science: "In science, this term describes a phenomenon, process, or concept in the natural world. Science is all about understanding how things work through observation and evidence. Could you tell me which specific scientific term you'd like defined?",
      history: "In history, this term refers to an event, period, or concept that helps us understand the past. History helps us learn from previous generations and understand how our present world came to be. Which historical term are you curious about?"
    };
    return definitions[category] || "That's a good term to learn about! To give you the best definition, could you provide a bit more context about what you're studying?";
  }
  ...
  if (questionType === 'example') {
    const examples = {
      ...
      science: "Here's a science example: The water cycle works like this — the sun heats water in the ocean, it evaporates into water vapor, rises and forms clouds (condensation), then falls as rain (precipitation), and eventually flows back to the ocean. This cycle repeats endlessly!",
      history: "Here's a historical example: When studying cause and effect, consider how the assassination of Archduke Franz Ferdinand in 1914 triggered a chain of events that led to World War I. Single events can have enormous historical consequences!"
    };
    return examples[category] || "Great question! Here's an example: in everyday life, we use knowledge from all subjects without realizing it — counting money uses math, cooking involves science with chemical reactions, and understanding holidays connects us to history.";
  }
...
function formatMathResponse(question) {
  const lower = question.toLowerCase();
  if (lower.includes('what')) return "That's a good math concept to learn about! Mathematics uses precise rules and formulas. Could you tell me which specific math topic you're studying — arithmetic, algebra, geometry, or fractions? I can explain it step by step.";
  if (lower.includes('how')) return "Here's how to approach math problems: first, understand what the question is asking. Then identify the important numbers and what operation to use (add, subtract, multiply, or divide). Finally, solve step by step and check your answer. Would you like to practice with a specific type of problem?";
  if (lower.includes('why')) return "Math rules exist because they're based on logical patterns! For example, we borrow in subtraction because we can't take a larger digit from a smaller one. Understanding the 'why' makes math much easier to remember.";
...
function formatScienceResponse(question) {
  const lower = question.toLowerCase();
  if (lower.includes('what')) return "Science helps explain the world around us! I can explain concepts in biology (living things), chemistry (matter and reactions), physics (energy and forces), and earth science (weather, water cycle, ecosystems). What topic interests you?";
  if (lower.includes('how')) return "In science, understanding how things work involves looking at the process step by step. For example, how does a plant grow? It needs sunlight, water, and nutrients from soil — then through photosynthesis, it converts these into energy to grow. What process would you like me to explain?";
  if (lower.includes('why')) return "Why questions in science lead to fascinating discoveries! The reason things happen the way they do usually comes down to scientific laws and principles. Like why is the sky blue? It's because blue light scatters more in our atmosphere than other colors. What's your 'why' question?";
...
function formatHistoryResponse(question) {
  const lower = question.toLowerCase();
  if (lower.includes('who')) return "Many important figures have shaped history! From Philippine heroes like José Rizal and Andrés Bonifacio to world leaders, inventors, and thinkers. Which time period or person are you interested in?";
  if (lower.includes('when')) return "Timing is key in history! Events are connected by cause and effect across different periods. The Philippines has a rich timeline from pre-colonial times, through 333 years of Spanish rule, American colonization, World War II, to independence and the present day. What period interests you?";
  if (lower.includes('where')) return "Geography and history are closely connected! Different places developed unique cultures and histories based on their location, resources, and interactions with neighboring regions. Which place are you curious about?";
  if (lower.includes('what')) return "History is full of interesting events and developments! The story of the Philippines includes ancient kingdoms, Spanish colonization, the Philippine Revolution, American rule, World War II, and the journey to becoming the vibrant nation it is today. What would you like to learn about?";

...
function suggestFollowUps(category, question, response) {
  ...
  if (lowerResponse.includes('step') || lowerResponse.includes('first') || lowerResponse.includes('next')) {
    specificFollowUps.push('Can you walk me through that again more slowly?');
  }
  if (category === 'math' && (lowerQuestion.includes('solve') || lowerQuestion.includes('calculate'))) {

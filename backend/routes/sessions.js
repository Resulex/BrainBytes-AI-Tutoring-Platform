...
router.post('/', optionalAuth, async (req, res) => {
  try {
    ...
    recordSessionCreated({
      ...
      authType: req.user ? 'authenticated' : 'anonymous',
...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    const shouldComplete = req.body.isActive === false && existingSession && existingSession.isActive !== false;
    ...
    if (shouldComplete) {
      const durationSeconds = existingSession.createdAt
        ? (Date.now() - new Date(existingSession.createdAt).getTime()) / 1000
        : 0;
      recordSessionCompleted({

...
const RECONNECT_RATE = parseFloat(getArg('--reconnect-rate', '0.05'));
...
async function simulateSession(workerId = 0) {
  ...
  const isMobile = PROFILE === 'mobile' || (PROFILE === 'mixed' && Math.random() < 0.4);
  const deviceType = isMobile ? 'mobile' : 'desktop';
  const connectionType = isMobile ? (Math.random() < 0.6 ? '4g' : '3g') : 'wifi';
  const reconnectAttempt = RECONNECT_RATE > 0 && Math.random() < RECONNECT_RATE;
  ...
  try {
    ...
    const headers = {
      ...
      'x-network-type': connectionType,
      ...(reconnectAttempt ? { 'x-reconnect-attempt': 'true' } : {}),
      ...(isMobile && Math.random() < 0.5 ? { 'x-low-bandwidth': 'true' } : {}),
    };

...
app.post('/api/messages', validate('message'), async (req, res, next) => {
  try {
    ...
    const connectionType = getConnectionType(req.headers);
    const reconnectAttempt =
      req.headers['x-reconnect-attempt'] === 'true' ||
      req.headers['x-reconnect-attempt'] === true ||
      req.headers['x-reconnect-attempt'] === '1';
    ...
    recordDataUsage({ deviceType, connectionType, bytes: requestBytes });
    if (reconnectAttempt) {
      recordConnectivityEvent({ deviceType });
    }
